...
# Âm thanh
background_music = pygame.mixer.Sound("/Users/huuhuynh/Desktop/Năm 2 kì 2/Trí tuệ nhân tạo/HideAndSeek/music/noi-nay-co-anh-trimmed.mp3")
game_music = pygame.mixer.Sound("/Users/huuhuynh/Desktop/Năm 2 kì 2/Trí tuệ nhân tạo/HideAndSeek/music/dung-lam-trai-tim-anh-dau-trimmed.mp3")
...
# Đường dẫn hình ảnh
image_path = "/Users/huuhuynh/Desktop/Năm 2 kì 2/Trí tuệ nhân tạo/HideAndSeek/Image/"
...
class Seeker:
    ...
    def backtracking_search(self, goal):
        ...
        def heuristic_order(pos):
            """Ưu tiên hướng gần mục tiêu hơn (heuristic Manhattan distance)."""
        ...
        def backtrack(current):
            ...
            # Sắp xếp các bước đi theo heuristic (ưu tiên gần mục tiêu)
            next_moves.sort(key=heuristic_order)
    ...
    def simple_hill_search(self, goal):
        start = (self.rect.x, self.rect.y)
        current = start
        path = [start]
...
def draw_results():
    # Vẽ bảng kết quả
    pygame.draw.rect(DISPLAYSURF, (0, 0, 0, 128), (400, 200, 460, 300))
    font = pygame.font.Font(None, 36)
    title = font.render("Rank", True, WHITE)
    DISPLAYSURF.blit(title, (550, 220))
...
def reset_game():
    global game_state, seeking_timer, hider_rect, seekers
    ...
    while True:
        ...
        if game_state == SEEKING:
            ...
            if seeking_timer <= 0:
                ...
                if not no_sound:
                    win_sound.play()  # Người trốn thắng nếu hết giờ
            ...
            if all(seeker.found for seeker in seekers):
                game_state = GAME_OVER
                if not no_sound:
                    lose_sound.play()  # Người trốn thua nếu bị tìm thấy

...
def draw_timer():
    ...
    if game_state == HIDING:
        timer_text = f"Thời gian trốn: {max(0, int(hiding_timer))}s"
        color = GREEN
...
def draw_instructions():
    ...
    if game_state == HIDING:
        instruction = "Find your save situation and press 'Enter' to pass the time" 
        color = WHITE
    elif game_state == SEEKING:
...
def reset_game():
    ...
    # Reset vị trí người tìm
    seeker_rect = pygame.Rect(0, 180, 30, 30)
...
while running:
    ...
    if game_state == MAIN_MENU:
        ...
        # Trong phần vẽ game (trước khi vẽ seeker)
        if game_state == SEEKING or game_state == GAME_OVER:
            draw_path()  # Vẽ đường đi trước
